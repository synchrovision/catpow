...
const getMainType = (conf) => (conf.extends && (Array.isArray(conf.extends) ? conf.extends[0] : conf.extends)) || conf.name;
const consoleError = (message) => console.error("JsonLD Block : " + message);
const convertDefaultValue = (value, conf) => {
	switch (conf.input) {
		case "datetime": {
...
const extractDefaultData = (conf, type = null) => {
	if (conf.type === "object") {
...
const filterEmptyData = (data, conf) => {
			...
			wp.apiFetch({ path: "/cp/v1/blocks/config/jsonld/types" }).then((res) => {
				...
				Object.keys(types).forEach((key) => {
					...
					if (Array.isArray(types[key].items)) {
						types[key].items.forEach((item) => fillConf(item, res));
					}
				});
				Object.values(types)
					.filter((type) => type.extends != null)
					.sort((a, b) => (Array.isArray(a.extends) ? a.extends.length : 0) - (Array.isArray(b.extends) ? b.extends.length : 0))
					.forEach((type) => fillType(type, res));
