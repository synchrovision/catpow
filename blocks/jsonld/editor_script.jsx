...
wp.blocks.registerBlockType('catpow/jsonld',{
	...
	edit({attributes,className,setAttributes}){
		...
		const extractDefaultData=useCallback((conf,type=null)=>{
			if(conf.type==='object'){
				...
				if(!itemConf.items){return conf.multiple?[{}]:{};}
		...
		const filterEmptyData=useCallback((data,conf)=>{
			...
			Object.keys(data).forEach((key)=>{
				if(Array.isArray(data[key])){
					...
					if(Object.keys(data[key]).length===(data[key].hasOwnProperty("@type")?1:0)){delete data[key];}
				}
			...
			return (
				<select className={classes()} value={data['@type']} onChange={(e)=>updateType(e.target.value)}>
					{Object.keys(types).map((type)=>(
						<option value={type} key={type}>{types[type].label || types[type].name}</option>
					))}
				</select>
			);
						...
						<div className={classes.object()}>
							{conf.items && conf.items.map((item)=>{
								return (
									<InputItem
										classes={itemClasses}
										data={index!==undefined?data[name][index]:data[name]}
										name={item.name}
										conf={item}
										level={level+1}
										key={item.name}
		...
		const InputItem=useCallback((props)=>{
			...
			const itemConf=useMemo(()=>{
				if(conf && conf.isDynamicType){
					if(!value['@type'] || !conf.confs[value['@type']]){value['@type']=Object.keys(conf.confs)[0];}
					...
					<div className={classes.inputs()}>
						{conf.isDynamicType && (
							<select className={classes.selecttype()} value={value['@type']} onChange={onChangeType}>
								{conf['@type'].map((type)=>(
									<option value={type} key={type}>{conf.confs[type].label || type}</option>
								))}
							</select>
						)}
						{itemConf.multiple?(
								...
								{data[name].map((item,index)=>(
									<div className={classes.inputs.group.item()} key={index}>
										...
										<div className={classes.inputs.group.item.control()}>
											{data[name].length>1 && (
												<div
													className={classes.inputs.group.item.control.decrease()}
													onClick={()=>removeItem(index)}
												></div>
											)}
											<div
												className={classes.inputs.group.item.control.increase()}
		...
		useEffect(()=>{
			wp.apiFetch({path:'/cp/v1/blocks/config/jsonld/types'}).then((res)=>{
				const fillConf=(conf)=>{
					...
					if(conf.items){conf.items.forEach(fillConf);}
					if(conf['@type']){
						if(Array.isArray(conf['@type'])){
							...
							else{consoleError(`@type ${conf['@type']} not found in formal schema`);}
						}
					}
					checkConf(conf);
					return conf;
				...
				const checkConf=(conf)=>{
					const errors=[];
					if(!conf.name){errors.push('require name property');}
					if(!conf.type){errors.push('require type property');}
					if(conf.type==='object'){
						if(conf.isDynamicType){
							if(!conf.confs){errors.push('require confs property for dinaymic type item');}
						}
						else{
							if(!conf.items){errors.push('require items property for input type object');}
						}
					}
					...
					{EditMode?(
						<div className={classes.editor()}>
							<div className={classes.editor.type()}>
								...
								/>
								{types[data['@type']] && (
									<a className={classes.editor.type.help()} href={types[data['@type']].url} target="_blank" rel="noopener noreferer"></a>
								)}
							</div>
						...
						<div className={classes.label()}>
							<div className={classes.label.icon()}><CP.ConfigIcon icon="json"/></div>
	...
	save({attributes,className,setAttributes}){
		return (
			<script type="application/ld+json">{attributes.json}</script>
		);
	}
});
