  ...
  edit({ attributes, setAttributes, className, isSelected, clientId }) {
    ...
    return (
      <>
        ...
        <div
          className={
            "formBlock cp-embeddedcontent" + (EditMode ? " editMode" : "")
          }
        >
          ...
          <PanelBody title="フォーム">
            <TreeSelect
              ...
              onChange={(content_path) => {
                const path = content_path.slice(
                  0,
                  content_path.lastIndexOf("/")
                );
                wp.apiFetch({ path: "cp/v1/" + path + "/template" }).then(
                  (template) => {
                    wp.data
                      .dispatch("core/block-editor")
                      .replaceInnerBlocks(clientId, CP.createBlocks(template));
                  }
                );
...
wp.blocks.registerBlockType("catpow/formblockcontent", {
  ...
  edit({ attributes, className, setAttributes, clientId }) {
    ...
    const parentClientId = wp.data
      ...
      .getBlockParentsByBlockName(clientId, "catpow/formblock")[0];
