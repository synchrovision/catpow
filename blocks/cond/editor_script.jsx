wp.blocks.registerBlockType("catpow/cond", {
  title: "🐾 Cond",
  description:
    "日時やログインユーザーによってコンテンツの表示が切り替わるコンテナです。",
  icon: "editor-code",
      ...
      <>
        <div className="cp-embeddedcontent">
          <div className="label">
            表示条件：
            {attributes.schedule}
            {attributes.is_user_logged_in != 0 &&
              "ログイン" +
                (attributes.is_user_logged_in == 1 ? "している" : "していない")}
          ...
          <PanelBody title="表示条件" icon="admin-generic">
            ...
            {attributes.is_user_logged_in == "1" && (
              <div className="sub">
                <TextareaControl
                  label="権限"
                  onChange={(current_user_can) =>
                    setAttributes({ current_user_can })
                  }
