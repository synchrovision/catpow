(() => {
	...
	wp.blocks.registerBlockType("catpow/section", {
		...
		save({ attributes }) {
			...
			const { SectionTag, HeadingTag, anchor, navIcon, classes, bodyClasses, headerClasses, titleClasses, vars, clipVars, headerVars, prefix, title, lead, titleImageCode, headerImageCode } =
				attributes;
			const states = CP.classNamesToFlags(classes);
			const { devices, imageKeys, imageSizes } = CP.config.section;
