...
CP.PlacedPictures.Edit = (props) => {
  ...
  const getPlaceStyle = useCallback((bnd, tgtBnd) => {
    ...
    if (px < 0.35) {
      ...
      style.left =
        "calc(50% + " +
        Math.pround((tgtBnd.left - bnd.left - bnd.width / 2) / remPx, 4) +
        "rem)";
    }
    if (py < 0.35) {
      ...
      style.bottom =
        Math.pround((bnd.bottom - tgtBnd.bottom) / remPx, 4) + "rem";
    } else {
      style.top =
        "calc(50% + " +
        Math.pround((tgtBnd.top - bnd.top - bnd.height / 2) / remPx, 4) +
        "rem)";
    }
    return style;
  }, []);
    ...
    >
      <BlockControls>
        ...
        {editMode &&
          ...
          pictures.length > currentItemIndexes.length && (
            <ToolbarButton
              icon="remove"
              label="remove"
              onClick={() => {
