...
CP.SelectColors = (props) => {
	...
	return (
		<div>
			<ColorPalette colors={colors} value={index > -1 ? colors[index].color : "rgba(0,0,0,0)"} onChange={onChangeOfColorPalette} disableCustomColors={true} />
			{index > -1 && (
				<Popover onClose={() => setIndex(-1)}>
					<ColorPicker color={colors[index].color} onChangeComplete={onChangeOfColorPicker} />
				</Popover>
			)}
		</div>
	);
