	...
	return (
		<BaseControl>
			{(state.events.length > 0 ? state.events : [{}]).map((event, index) => (
				<EventInputCard title={title} event={event} index={index} processer={processer} canRemove={state.events.length > 1} key={index} dispatch={dispatch} />
			))}
		</BaseControl>
	);
};

const EventInputCard = (props) => {
