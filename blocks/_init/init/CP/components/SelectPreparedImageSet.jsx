export const SelectPreparedImageSet = ({ className, name, value, color = 0, onChange, ...otherProps }) => {
	...
	return (
		<ul className={"cp-selectpreparedimageset " + name + " " + className} {...otherProps}>
			{Object.keys(state.imagesets).map((key) => {
				...
				return (
					<li className={"item " + (value == url ? "active" : "")} key={key}>
						<img src={url} alt={imageset[0].alt} onClick={() => dispatch({ type: "update", imageset })} />
					</li>
				);
			})}
		</ul>
