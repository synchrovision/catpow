	...
	const Item = useCallback((props) => {
		...
		return (
			<div className={classes(flagsToClassNames({ isCustomized }))}>
				<div className={classes.chip()} onClick={() => setIsOpen(!isOpen)} style={{ backgroundColor: color }}>
	...
	return (
		<div className={classes()}>
			...
			<div className={classes.items({ "is-active": useCustomColor })}>
				{roles.map((role) => (
					<Item role={role} classes={classes.items.item} onChange={onChange} color={colors[role.key] || false} originalColor={originalColors[role.key]} key={role.key} />
				))}
			</div>
		</div>
	);
