...
CP.SelectClassPanel = (props) => {
  ...
  const selectiveClasses = useMemo(() => {
    ...
    return triggerClasses.item[
      Object.keys(triggerClasses.item).find((value) => blockStates[value])
    ];
  ...
  const SelectClass = useCallback(({ prm }) => {
    const { props, item, states, save, saveClasses, saveCss } = useContext(
    ...
    if (prm.hasOwnProperty("cond")) {
      ...
      if (
        Array.isArray(prm.cond) &&
        prm.cond.some((className) => !states[className])
      ) {
        return false;
      }
          ...
          case "gridbuttons": {
            if (prm.sub) {
              if (prm.sub[JSON.parse(props.attr[prm.json])[prm.key]]) {
                let sub = [];
                prm.sub[JSON.parse(props.attr[prm.json])[prm.key]].forEach(
                  (prm) => {
              ...
              <CP.SelectPreparedImageSet
                ...
                onChange={(imageset) => {
                  imageset.forEach((image) => {
                    ...
                    const media = CP.getMediaQueryKeyForDevice(device);
                    styleDatas[prm.css][media] =
                      styleDatas[prm.css][media] || {};
                    styleDatas[prm.css][media][sel] =
                      styleDatas[prm.css][media][sel] || {};
                    styleDatas[prm.css][media][sel].borderStyle = "solid";
                    styleDatas[prm.css][media][sel].borderImage =
                      "url(" +
                      image.url +
                      ") fill " +
                      slice +
      ...
      if (prm === "color") {
        ...
        rtn.push(
          <CP.SelectPatternClass
            ...
            set={props.set}
            attr={props.attr}
            selected={Object.keys(states).find((key) =>
              /^pattern\d+/.test(key)
            )}
            onChange={(pattern) => {
        ...
        const EventInputs = useMemo(
          () => wp.hooks.applyFilters("catpow.EventInputs", [], { item, save }),
          [item, save]
        );
