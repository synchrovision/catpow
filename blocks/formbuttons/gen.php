...
<?php foreach((array)$attr['items'] as $item): ?>
	<li class="item <?=$item['classes']??''?>">
		<div class="button" data-action="<?=$item['action']??''?>" data-callback="<?=$item['callback']??''?>" data-target="<?=$item['target']??''?>" ignore-message="<?=$item['ignoreMessage']??''?>"<?=empty($item['event'])?'':sprintf(' data-event="%s"',$item['event'])?>>
