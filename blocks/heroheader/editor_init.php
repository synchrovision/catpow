<?php
$data=[
	'label'=>__('背景スライドショー','catpow'),
	'input'=>'select',
	'key'=>'element',
	'sub'=>[]
];
foreach(cp::glob('elements/slide-show-*/schema.json') as $key=>$schema_json){
	$schema=new Catpow\util\schema($schema_json);
	$element=basename(dirname($schema_json));
	$title=$schema->title??substr($element,11);
...
wp_localize_script('catpow','heroheaderSelectiveClasses',$data);