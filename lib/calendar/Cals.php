<?php
/*
複数のCalendarオブジェクトのグループとなるクラス
カレンダー出力を補助するプロパティやメソッドをもつ

■プロパティ

■メソッド
range	カレンダーの範囲(set,get)
data		カレンダーのデータ(set,get)
			[date,class,type,label,name,checked,icon,href,html]
length	カレンダーの月数(get)


■備考
rangeが設定されていないときにdateのセットされたdataを設定すると
データ範囲に応じてrangeが自動的に設定されるので
dateのセットされたdataとrangeを両方設定する場合は
rangeを先に設定するほうが処理にムダがない

dataにdateがない場合
rangeを設定しないとカレンダーが生成されない

各日付のセルに入る要素は
p.holiday
p.date
div.events#[CalsN-Ymd]
-div.event.[class] 
--label.[(class)] input#[Cals(N)-(Ymd)-input-(class)]
--img.icon
--a.link#[Cals(N)-(Ymd)-link-(class)]
--div.html#[Cals(N)-(Ymd)-html-(class)]

*/
include(dirname(__FILE__).'/calendar.php');
include(dirname(__FILE__).'/Cals_loop_format.php');
...
class Cals{
	...
	public function write($format='div'){
		...
		echo($format['after']);
		if(!empty($this->_prm['legend']))$this->write_legend();
	}
