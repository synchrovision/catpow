	...
	const InputComponent = useMemo(() => {
		if (layout === "table") {
			return (props) => {
				...
				const sortedColsKeys = Object.keys(cols)
					.filter((key) => !cols[key].hidden)
					.sort((key1, key2) => (cols[key1].order || 10) - (cols[key2].order || 10));

				return (
					<Catpow.TableInput size={size} labels={sortedColsKeys.map((key) => cols[key].title || key)} onAddItem={onAddItem} onCopyItem={onCopyItem} onMoveItem={onMoveItem} onRemoveItem={onRemoveItem}>
						{props.agent.items.map((item) =>
							sortedColsKeys.map((col) => {
		...
		return (props) => {
			const { agent } = props;
			return (
				<Catpow.ArrayInput size={size} onAddItem={onAddItem} onCopyItem={onCopyItem} onMoveItem={onMoveItem} onRemoveItem={onRemoveItem}>
					{props.agent.items.map((item) => (
