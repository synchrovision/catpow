			...
			return (
				<tr className={classes()}>
					{children.map((child, index) => (
						<td className={classes.td("is-input")} key={index}>
							{child}
						</td>
					))}
					<td className={classes.td("is-control")} key={index}>
						<div className={classes.td.controls()}>
							{index > 0 ? <div className={classes.td.controls.button("is-up")} onClick={() => onMoveItem(index, index - 1)}></div> : <div className={classes.td.controls.spacer()} />}
							{index < length - 1 ? <div className={classes.td.controls.button("is-down")} onClick={() => onMoveItem(index, index + 1)}></div> : <div className={classes.td.controls.spacer()} />}
							{length > 1 ? <div className={classes.td.controls.button("is-remove")} onClick={() => onRemoveItem(index)}></div> : <div className={classes.td.controls.spacer()} />}
		...
		<div className={classes()}>
			<table className={classes.table()}>
					...
					<thead className={classes.table.thead()}>
						<tr className={classes.table.thead.tr()}>
							{labels.map((label, index) => (
								<th className={classes.table.thead.tr.th()} key={index}>
									{label}
								</th>
							))}
				...
				<tbody className={classes.table.tbody()}>
					{children.map((child, index) => (
						<Row classes={classes.table.tbody.tr} index={index} length={children.length} key={index}>
							{child}
						</Row>
					))}
				</tbody>
