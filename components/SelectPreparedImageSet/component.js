(() => {
  ...
  Catpow.SelectPreparedImageSet = (props) => {
      ...
      (state2, action) => {
        switch (action.type) {
          case "update":
            ...
            if (state2.imageset) {
              if (onSelect) {
                onSelect(state2.imageset);
              }
              onChange(
                state2.imageset.map((item) => {
    ...
    return /* @__PURE__ */ wp.element.createElement("ul", { className: classes("is-" + name), ...otherProps }, Object.keys(state.imagesets).map((key) => {
      ...
      return /* @__PURE__ */ wp.element.createElement("li", { className: classes.item({ "is-active": value == url }), key }, /* @__PURE__ */ wp.element.createElement("img", { className: classes.item._img(), src: url, alt: imageset[0].alt, onClick: () => dispatch({ type: "update", imageset }) }));
    }));
  };
})();
