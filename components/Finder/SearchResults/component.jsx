		...
		useCallback(({ state, info }) => {
			...
			return (
				<ul className={flagsToWords(flags)}>
							...
							<li className={"item" + (row._selected ? " selected" : "")} key={"row" + row._id}>
								<div className="contents">
									<div className="control">
										<Catpow.CheckBox selected={row._selected} onChange={(selected) => dispatch({ type: selected ? "selectRow" : "deselectRow", row })} />
									</div>
									{flags.hasImages && (
										<div className="images">
											{roleGroups.images.map((role) => {
												...
												return colsToShowByRole[role].map((col) => <Catpow.Output conf={col} {...row[col.name]} key={col.name} />);
											})}
										</div>
									)}
									<div className="texts">
										...
										<div className="focus" onClick={() => dispatch({ type: "focusItem", row })}>
											{__("詳細を見る", "catpow")}
										</div>
									</div>
								</div>
					...
					{[...Array(10).keys()].map((i) => (
						<li className="spacer" key={"spacer" + i}></li>
					))}
				</ul>
			);
		}, []);
						...
						<tbody className="body">
							{state.itemsInPage.map((row) => (
								<tr className={"row" + (row._selected ? " selected" : "")} key={row._id}>
									...
									<th className="focus">
										<div className={"icon dashicons dashicons-admin-page"} onClick={() => dispatch({ type: "focusItem", row })}></div>
									</th>
									{state.colsToShow.map((col) => (
										<td className="cell" key={col.name}>
