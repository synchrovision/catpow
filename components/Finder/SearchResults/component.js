Catpow.Finder.SearchResults = function (props) {
	...
	var ListView = props.listView || useCallback(function (_ref) {
		...
		return wp.element.createElement(
			...
			state.itemsInPage.map(function (row) {
				return wp.element.createElement(
					'li',
					{ className: 'item' + (row._selected ? ' selected' : ''), key: 'row' + row._id },
					wp.element.createElement(
						...
						wp.element.createElement(
							'div',
							{ className: 'control' },
							wp.element.createElement(Catpow.CheckBox, { selected: row._selected, onChange: function onChange(selected) {
									return dispatch({ type: selected ? 'selectRow' : 'deselectRow', row: row });
								} })
						),
						flags.hasImages && wp.element.createElement(
							'div',
							{ className: 'images' },
							roleGroups.images.map(function (role) {
								...
								return colsToShowByRole[role].map(function (col) {
									return wp.element.createElement(Catpow.Output, babelHelpers.extends({ conf: col }, row[col.name]));
								});
							})
						),
						wp.element.createElement(
							...
							wp.element.createElement(
								'div',
								{ className: 'focus', onClick: function onClick() {
										return dispatch({ type: 'focusItem', row: row });
									} },
								__('詳細を見る', 'catpow')
							)
						)
			...
			[].concat(babelHelpers.toConsumableArray(Array(10).keys())).map(function (i) {
				return wp.element.createElement('li', { className: 'spacer', key: 'spacer' + i });
			})
		);
	}, []);
		...
		return wp.element.createElement(
			...
			wp.element.createElement(
				...
				wp.element.createElement(
					...
					wp.element.createElement(
						...
						wp.element.createElement(Catpow.CheckBox, {
							...
							onChange: function onChange(checked) {
								state.itemsInPage.map(function (item) {
					...
					wp.element.createElement('th', { className: 'focus' }),
			...
			wp.element.createElement(
				...
				state.itemsInPage.map(function (row) {
					return wp.element.createElement(
						'tr',
						{ className: "row" + (row._selected ? ' selected' : ''), key: 'row' + row._id },
						wp.element.createElement(
							'th',
							{ className: 'control' },
							wp.element.createElement(Catpow.CheckBox, { selected: row._selected, onChange: function onChange(selected) {
						...
						wp.element.createElement(
							'th',
							{ className: 'focus' },
							wp.element.createElement('div', { className: "icon dashicons dashicons-admin-page", onClick: function onClick() {
									return dispatch({ type: 'focusItem', row: row });
								} })
						),
						state.colsToShow.map(function (col) {
							return wp.element.createElement(
								'td',
								{ className: 'cell' },
								wp.element.createElement(Catpow.Output, babelHelpers.extends({ conf: col }, row[col.name]))
							);
						})
					);
				})
	...
	return wp.element.createElement(
		...
		wp.element.createElement(
			...
			state.items.length > 0 ? wp.element.createElement(
				'div',
				{ className: "FinderSearchResultsItems " + state.layout + "-view", depth: 1, page: state.page, view: state.layout },
				el(state.layout === 'table' ? TableView : ListView, { state: state, dispatch: dispatch, info: info })
			) : wp.element.createElement(
				'div',
				{ className: "message", depth: 1, page: 1, view: 'message' },
				__('検索結果なし', 'catpow')
			)
		)
	);
