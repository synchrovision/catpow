(() => {
  ...
  Catpow.Finder.SearchResults = (props) => {
    ...
    const flagsToWords = useCallback(
      (classes) => Object.keys(classes).filter((key) => classes[key]).join(" "),
      []
    );
    ...
    const TableView = props.tableView || useCallback(
      ({ state: state2 }) => {
        ...
        )), /* @__PURE__ */ wp.element.createElement("th", { className: "focus" }), state2.colsToShow.map((col) => /* @__PURE__ */ wp.element.createElement("th", { className: "cell", key: col.name }, col.label, /* @__PURE__ */ wp.element.createElement(
          "span",
          {
            className: "sort sort-" + (state2.sort[col.name] || "none"),
            onClick: () => {
    ...
    return /* @__PURE__ */ wp.element.createElement("div", { className: "cp-finder-searchresults" }, /* @__PURE__ */ wp.element.createElement(Catpow.Transition, { type: state.transition }, state.items.length > 0 ? /* @__PURE__ */ wp.element.createElement("div", { className: "cp-finder-searchresults-items " + state.layout + "-view", depth: 1, page: state.page, view: state.layout }, el(state.layout === "table" ? TableView : ListView, { state, dispatch, info })) : /* @__PURE__ */ wp.element.createElement("div", { className: "message", depth: 1, page: 1, view: "message" }, __("\u691C\u7D22\u7D50\u679C\u306A\u3057", "catpow"))));
  };
})();
