	...
	return (
		<Catpow.Popup open={state.open} closeOnClickAway={false} onClosed={() => onComplete(state.values)}>
			<Catpow.ModalFormContext.Provider value={{ state, dispatch }}>
				<div className={"cp-modalform"}>{children}</div>
...
Catpow.ModalForm.Input = (props) => {
	...
	const onChange =
		props.onChange ||
		useCallback(({ state, dispatch, name, value }) => {
			dispatch({ type: "setValue", name, value });
		});

	return (
		<InputComponent value={state.values[name]} onChange={(value) => onChange({ state, dispatch, name, value })} {...otherProps}>
			{children}
		</InputComponent>
	);
};
