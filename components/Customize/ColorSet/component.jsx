		...
		if (/^#(\w{6}|\w{8})$/.test(color)) {
			return (
				color
					.match(/#?(\w{2})(\w{2})(\w{2})/)
					.slice(1)
					.reduce((p, c, i) => p + parseInt(c, 16) * [3, 6, 2][i], 0) < 0x600
			);
		}
			...
			return (
				<div className="cp-colorset">
					...
					<div className="cp-colorset-palette">
						{Object.keys(roles).map((role) => (
							<Palette role={role} value={colors} open={role === activeRole} onClick={() => setActiveRole(role === activeRole ? null : role)} key={role} />
						))}
					</div>
