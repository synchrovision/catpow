	...
	return (
		<g className="ChartText">
			{keys.map((key) => {
				...
				return (
					<text className={key} x={pos.x} y={pos.y}>
						{props[key]}
					</text>
				);
			})}
			<g className={"rowLabel"}>
				{props.rows.map((item, i) => {
					...
					return (
						<text className={item.classes.replace("row", "")} x={pos.x} y={pos.y}>
							{item.label}
						</text>
					);
				})}
				...
				{props.cols.map((col, c) => {
					...
					return (
						<g className="ledgendItem">
							<rect className={cols[c].classes.replace("col", "ledgendRect")} x={pos.x} y={pos.y - 6} width={12} height={12} />
							<text className="ledgendText" x={pos.x + 15} y={pos.y}>
								{col.label}
							</text>
						</g>
					);
				...
				<g className="vals">
					{props.rows.map((row, r) => {
						return row.vals.map((val, c) => {
							...
							return (
								<text className={row.classes.replace("row", "") + " " + props.cols[c].classes.replace("col", "val")} x={pos.x} y={pos.y}>
									{val.value}
									{props.hasUnit && <tspan className="unit">{props.unit}</tspan>}
								</text>
							);
						});
					})}
