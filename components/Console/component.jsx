...
Catpow.Console = (props) => {
				...
				() => ({
					text: (props) => (
						<input
							type="text"
							onChange={(e) => {
								dispatch({ type: "setData", name, value: e.currentTarget.value });
							}}
					...
					select: (props) => (
						<SelectBox
							onChange={(value) => {
								dispatch({ type: "setData", name, value });
							}}
	...
	const submit = useCallback((action, data) => {
		wp.apiFetch({ path: `/cp/v1/${path}/${action}`, method: "POST", data })
			.then((res) => {
	...
	return (
		<div className={className}>
			<div className={className + "-controls"}>{props.controls.map((itemProps, key) => el(ControlItem, { ...itemProps, state, dispatch, key }))}</div>
			<div className={className + "-results"}>{state.results.map((itemProps, key) => el(ResultItem, { ...itemProps, key }))}</div>
		</div>
	);
};
