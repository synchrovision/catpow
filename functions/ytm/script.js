...
window.Catpow.yjad={
	...
	parseEventString:function(str){
		...
		if(!matches){return {};}
		var rtn={};
		if(matches[1]){rtn.io=matches[1];}
		if(matches[2]){rtn.label=matches[2];}
		if(matches[3]){rtn.transaction_id=matches[3];}
		if(matches[4]){rtn.value=matches[4];}
		rtn.event=matches[5] || 'click';
		return rtn;
	},
	createEventValue:function(datas){
		return datas.filter((data)=>!!data.label).map(window.Catpow.yjad.createEventString).join(' + ');
	},
	createEventString:function(data){
		if(!data.label)return '';
		var rtn=data.label;
		if(data.io){rtn=`【${data.io}】${rtn}`;}
		if(data.transaction_id){rtn+=`«${data.transaction_id}»`;}
		if(data.value){rtn+='#'+data.value;}
		if(data.event && data.event!=='click'){rtn+='@'+data.event;}
		return rtn;
	},
	send:function(data){
		...
		if(data.io){config.yahoo_ydn_conv_io=data.io;}
		if(data.transaction_id){config.yahoo_ydn_conv_transaction_id=data.transaction_id;}
		if(data.value){config.yahoo_ydn_conv_value=data.value;}
