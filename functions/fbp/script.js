...
window.Catpow.fbp = {
	...
	eventTypes: {
		AddPaymentInfo: {
			label: "支払い情報の追加",
			options: ["content_category", "contents", "currency", "value"],
			require: [],
		},
		AddToCart: {
			label: "カートに追加",
		...
		CompleteRegistration: {
			label: "登録完了",
			options: ["content_name", "currency", "status", "value"],
			require: [],
		},
		...
		Lead: {
			label: "リード",
			options: ["content_category", "content_name", "currency", "value"],
			require: [],
		},
		Purchase: {
			label: "購入",
		...
		StartTrial: {
			label: "トライアル開始",
			options: ["currency", "predicted_ltv", "value"],
			require: [],
		},
	...
	eventParams: {
		...
		content_type: {
			type: "gridbuttons",
			label: "タイプ",
			options: { product: "製品", product_group: "製品グループ" },
		},
		contents: {
			type: "data",
			label: "内容",
		...
		custom: {
			type: "data",
			label: "カスタムパラメータ",
			cols: { label: { label: "ラベル" }, value: { label: "値" } },
		},
	...
	createEventValue: function (datas) {
		return datas
			...
			.map(window.Catpow.fbp.createEventString)
			.join(" + ");
	},
	createEventString: function (data) {
		...
		if (data.contents) {
			...
			rtn += data.contents
				...
				.join(",");
			rtn += "」";
			if (data.content_type === "product_group") {
				rtn += "*";
			}
		}
		if (data.currency) {
			...
			} else if (data.predicted_ltv) {
				rtn += currencyUnits[data.currency] + "~" + data.predicted_ltv;
			}
		}
		if (data.search_string) {
			rtn += "##" + data.search_string + "##";
		}
	...
	send: function (data) {
		...
		eventParams.map(function (eventParam) {
			if (data[eventParam] !== undefined) {
				prm[eventParam] = data[eventParam];
			}
		});
		if (data.custom) {
