...
window.Catpow.ga = {
	...
	eventTypes: {
		...
		close_convert_lead: { label: "購入を完了", options: ["currency", "value"], require: ["currency", "value"] },
		close_unconvert_lead: { label: "購入を中止", options: ["currency", "value", "unconvert_lead_reason"], require: ["currency", "value"] },
		disqualify_lead: { label: "購入見込みなし", options: ["currency", "value", "disqualified_lead_reason"], require: ["currency", "value"] },
		earn_virtual_currency: { label: "仮想通貨の授与", options: [], require: [] },
		generate_lead: { label: "見込み顧客の発掘", options: ["currency", "value", "lead_source"], require: ["currency", "value"] },
		...
		select_content: { label: "コンテンツを選択", options: ["content_type", "content_id"], require: [] },
	...
	eventParams: {
		...
		unconvert_lead_reason: { type: "text", label: "理由", extra: true },
		disqualified_lead_reason: { type: "text", label: "理由", extra: true },
		lead_source: { type: "text", label: "発掘元", extra: true },
		content_type: { type: "text", label: "コンテンツタイプ", extra: true },
		content_id: { type: "text", label: "コンテンツ識別子", extra: true },
	...
	parseEventString: function (str) {
		...
		if (!window.Catpow.ga.EventStringPattern) {
			...
			var patternString =
				...
				"(?::([\\d\\w_]+?))?" + //category
				"(?:«([^»]+?)?»)?" + //event_label
				`(?:([${currencyUnits}])(\\d+(?:\\.\\d+)?))?` + //currency value
				"(?:「(.+)」)?" + //items
				"(?:【(.+)】)?" + //custom
				"(?:［(.+)］)?" + //extra
				"(?:@(\\w+))?" + //event
		...
		var rtn = {};
		var keys = ["eventType", "category", "event_label", "currency", "value", "items", "custom", "extra", "event", "send_to"];
		keys.map(function (key, index) {
			if (matches[index + 1] !== undefined) {
				rtn[key] = matches[index + 1];
			}
		});
		if (rtn.currency) {
		...
		if (rtn.extra) {
			rtn.extra = rtn.extra.split(", ").reduce(function (p, c) {
	...
	createEventValue: function (datas) {
		return datas
			.filter((data) => !!data.eventType)
			.map(window.Catpow.ga.createEventString)
			.join(" + ");
	},
	createEventString: function (data) {
		...
		if (data.event_label) {
			rtn += "«" + data.event_label + "»";
		}
		if (data.currency && data.value) {
			rtn += currencyUnits[data.currency] + data.value;
		}
		if (data.items) {
			rtn += "「";
			rtn += data.items
		...
		if (data.extra) {
			rtn += "［";
			rtn += Object.keys(data.extra)
				.map((key) => key + " : " + data.extra[key])
				.join(", ");
			rtn += "］";
		}
	...
	send: function (data) {
		...
		var prm = {};
		if (data.category) {
			prm.event_category = data.category;
		}
		if (data.event_label) {
			prm.event_label = data.event_label;
		}
		if (data.currency) {
			prm.currency = data.currency;
		}
		if (data.value) {
			prm.value = data.value;
		}
		if (data.items) {
			prm.items = data.items;
		}
		if (data.custom) {
